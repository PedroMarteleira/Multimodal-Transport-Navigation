...
/**
 * Strategy that chooses the transport with the lowest Duration in the available ones
 *
 * @author Pedro Marteleira (202300334@estudantes.ips.pt)
 */
public class LowestDurationStrategy implements TransportStrategy {
     ...
     */
    public LowestDurationStrategy(Collection<String> availableTransports) {
    ...
    public double getWeight(Route route) throws MissingTransportException {
        return Objects.requireNonNull(route).getTransportInformation(getTransport(route)).getDuration();
    }
    ...
    public String getTransport(Route route) throws MissingTransportException {
        return route.getAvailableTransports().stream().filter(availableTransports::contains).min((a, b) -> {
            ...
            return (int) (ta.getDuration() * FACTOR - tb.getDuration() * FACTOR);
